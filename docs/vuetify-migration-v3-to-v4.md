...
> `GrowthChart.vue` や `YearlyTable.vue` で `color="accent"` を使用しているため、
...
| リスク | 対応策 |
|---|---|
| Vuetify 4 がまだアルファ・ベータ版の場合、本番利用には注意が必要 | リリースノートで安定版リリース状況を確認してから移行する |
| `accent` カラーが廃止された場合、複数コンポーネントのデザインに影響が出る | カスタムカラー定義を継続するか、`primary` / `secondary` に統一する |
